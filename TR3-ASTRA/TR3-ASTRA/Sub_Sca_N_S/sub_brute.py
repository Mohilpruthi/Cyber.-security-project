...
def get_input():
    ...
    threads = input("Enter the number of threads (Optional, press Enter for default): ")
    wordlist = input("Enter the path to the wordlist file (Optional, press Enter for default): ")
    verboses = input("Enable Verbose Mode? (Optional, press Enter for default): ")
    fastest = input("Enable Fastest Mode? (Optional, press Enter for default): ")

    return domain, threads, wordlist, verboses, fastest
...
def get_arg():
    ...
    parser.add_argument("-w[OPTIONAL]", "--wordlist", dest="wordlist", help="Specify Custom Wordlist To Use(Default /usr/share/subscannon/default.txt)")
    parser.add_argument("-v[OPTIONAL]", "--verbose", dest="verboses", help="For Verbose Output", nargs='?', const='')
    parser.add_argument("-f[OPTIONAL]", "--fastest", dest="fastest", help="For Fastest Output , -v -t does Not Work With Fastest Mode (Best For Large Domains)", nargs='?', const='')
...
domain, threads, wordlist, verboses, fastest = get_input()
...
arg = get_arg()
domain = arg.domain if not domain else domain
threads = arg.threads if not threads else threads
wordlist = arg.wordlist if not wordlist else wordlist
verboses = arg.verboses if not verboses else verboses
fastest = arg.fastest if not fastest else fastest
